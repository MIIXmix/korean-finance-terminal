...
주식 투자자(특히 주린이)를 위한 고밀도 한국어 금융 터미널입니다. 미국 주식, ETF, 지수, 금리, 원자재, 환율, 한국 주식, 섹터 히트맵, 뉴스, SEC 공시, DART 공시, 옵션, 포트폴리오(원화 환산 비중·수익률), 모의 주문(내부 + KIS 모의투자), Gemini 선택 연결을 한 화면에서 다룹니다.

**개인 사용자용 로컬 앱**입니다. 각자 자기 PC에서 실행하고, 마스터 비밀번호로 앱 전체를 잠급니다. 데이터·API 키는 모두 본인 기기에만 저장됩니다.

**책임투자 설계**: 투자가 너무 쉬워 보이지 않게 위험 고지·면책, 거래비용 안내(수수료·세금 별도), 집중도 경고를 곳곳에 둡니다. 매매 권유가 아니라 정보·기록용입니다.
