...
주식 투자자(특히 주린이)를 위한 고밀도 한국어 금융 터미널입니다. 미국 주식, ETF, 지수, 금리, 원자재, 환율, 한국 주식, 섹터 히트맵, 뉴스, SEC 공시, DART 공시, 옵션, 포트폴리오(원화 환산 비중·수익률), Paper Trading, Gemini 선택 연결을 한 화면에서 다룹니다.

**개인 사용자용 로컬 앱**입니다. 각자 자기 PC에서 실행하고, 마스터 비밀번호로 앱 전체를 잠급니다. 데이터·API 키는 모두 본인 기기에만 저장됩니다.
...
- **단계별 온보딩 마법사**: 첫 설정 직후 ①앱 소개 ②Gemini 키 발급 안내 ③DART 키(선택) ④완료 순서로 안내합니다. 키는 모두 선택이며 건너뛸 수 있습니다.
- **키 없이도 동작**: 시세·차트·한국 뉴스는 키 없이 바로 사용됩니다. 키는 AI 번역/분석(Gemini), 한국 전자공시(DART)를 추가로 켭니다.

## 현재 구현

- 포트폴리오: 종목 검색(회사명/티커), 시장 선택(미국/코스피/코스닥), 보유 추가·수정·삭제(중복 종목 가중평균 합산), **원화(KRW) 환산** 총 평가/원금/손익/수익률, 종목별 비중 도넛, 섹터/국가/통화 배분
- 환율 위젯: USD·EUR·CNY/KRW, JPY는 100엔 기준, USD/JPY, EUR/USD (60초 갱신)
- 섹터 히트맵: 코스피·나스닥100 트리맵
- 한국 종목: KOSPI/KOSDAQ 목록을 네이버 금융 공개 페이지에서 지연 조회, 실패 시 내장 스냅샷 fallback
- 사용자별 레이아웃 저장, API 키 암호화 저장, Paper Trading 주문 기록
...
- 주문: 기본 Paper Trading. 실거래 API 경로는 서버 플래그가 꺼져 있으면 차단
...
| 브로커 | 구조/문서/보안 가드 | Alpaca/IBKR/KIS별 키 | 기본 Paper Trading |
